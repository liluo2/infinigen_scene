...
def export_curr_scene(
    ...
    if individual_export:
        ...
        scene_list = []
        save_json = {}
        save_json["stage_instance"] = {
            "template_name": f"{output_folder}",
            "translation_origin": "asset_local"
        }
        save_json["translation_origin"] = None
        save_json["object_instances"] = []

        object_set = [bpy.data.collections['unique_assets'].objects, bpy.data.collections['unique_assets:windows'].objects, bpy.data.collections['unique_assets:staircases'].objects, bpy.data.collections['unique_assets:door_casings'].objects, bpy.data.collections['unique_assets:doors'].objects]
        for assets in object_set:
            for obj in assets:
            # for obj in bpy.data.objects:
                if (
                    obj.type != "MESH"
                    ...
                    or len(obj.data.vertices) == 0
                    or obj not in list(bpy.context.view_layer.objects)
                    # or obj not in bpy.data.collections['unique_assets']
                ):
                    continue

                # rotation: 4D
                asset_dict = {"template_name": None, "translation": None, "rotation": None, "non_uniform_scale": None, "motion_type": None}
                
                # Obtain the required things here.
                bbox_center = get_global_bounds(obj) 
                center, bbox_size = get_bounding_box_center_size(obj)
                ...
                # TODO: motion_type is STATIC by default.
                # TODO: what is "non_uniform_scale"?
                asset_dict.update({"template_name": obj_name.split("_spawn_asset")[0], "translation": bbox_center, "rotation": rotation, "non_uniform_scale": bbox_size, "motion_type": "STATIC"})
                
                save_json["object_instances"].append(asset_dict)
        ...
        room_objects_set = [bpy.data.collections['unique_assets:room_ceiling'].objects, bpy.data.collections['unique_assets:room_floor'].objects, bpy.data.collections['unique_assets:room_wall'].objects]
        for room_objects in room_objects_set:
            for obj in room_objects:
                # TODO: the exterior, ... of the scene should be joined.
                if (
                    obj.type != "MESH"
                    or obj.hide_render
                    or len(obj.data.vertices) == 0
        ...
        json_path = export_folder.parent / "scene.json"
...
def export_scene_with_objects(
    ...
    **kwargs,
    ...
    folder = output_folder / f"export_{input_blend.name}"
    folder.mkdir(exist_ok=True, parents=True)
    export_curr_scene_with_objects(folder, **kwargs)
...
@gin.configurable
def export_curr_scene_with_objects(
    output_folder: Path,
    format="usdc",
    image_res=1024,
    vertex_colors=False,
